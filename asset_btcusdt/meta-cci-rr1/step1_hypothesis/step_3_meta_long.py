...
long_labels['long_bin'] = 0
long_labels.loc[(long_labels['bin']==1)&(long_labels['side']==1), 'long_bin'] = 1

long_labels['long_bin'].value_counts()

#%%
long_labels

#%%
long_labels_train = long_labels[:"2022-01-12"]
long_labels_oos   = long_labels["2022-01-14":]

#%%
# ML friendly dataset for long only model

X_train = df.loc[long_labels_train.index][trainable_features].copy()
X_train.replace([np.inf, -np.inf], np.nan, inplace=True)
X_train.fillna(0, inplace=True)
y_train = long_labels_train['long_bin']

X_test  = df.loc[long_labels_oos.index][trainable_features].copy()
X_test.replace([np.inf, -np.inf], np.nan, inplace=True)
X_test.fillna(0, inplace=True)
y_test  = long_labels_oos['long_bin']
...
return_based_sample_weights_long = get_weights_by_return(
    triple_barrier_events.loc[X_train.index], 
    df.loc[X_train.index, 'Close'],
    num_threads=1)

return_based_sample_weights_long.plot()
...
importance = feature_importance_clustered_MDA(
    ...
    sample_weight=return_based_sample_weights_long, 
    scoring="f1",
    n_splits=10
    )
    ...
    for m_depth in parameters['max_depth']:
        for n_est in parameters['n_estimators']:
            ...
            temp_score_base = fml.cross_validation.ml_cross_val_score(clf, X_data, y_data, cv_gen, scoring=scoring,
                                                                    sample_weight_train=return_based_sample_weights_long.values)
...
calibrated_rf = CalibratedClassifierCV(rf_uncalibrated, cv=4, method="sigmoid") 
calibrated_rf.fit(
    X_train[selected_cmda_features], y_train,
    sample_weight=return_based_sample_weights_long
    )
...
eval_df['position'] = 0
eval_df['position'] = long_labels_oos['side']
...
#%%
long_df = eval_df[eval_df['position']==1].copy()
long_df['prob'] = 0.0
long_df['prob'] = rf_meta.predict_proba(long_df[selected_cmda_features])[:,1]
eval_df['prob'] = long_df['prob']

long_df['prob'].hist(bins=300)
...
long_df = eval_df[eval_df['position']==1].copy()
long_df['prob'] = 0.0
long_df['prob'] = calibrated_rf.predict_proba(long_df[selected_cmda_features])[:,1]
eval_df['prob'] = long_df['prob']
...
class CCIMetaLongStrategy(Strategy):
    def init(self):
        super().init()
    def next(self):
        super().next()
...
class CCIMetaLongFilteredStrategy(Strategy):
    def init(self):
        super().init()
    def next(self):
        super().next()
...
joblib.dump(rf_meta, "../model/btcusdt_rf_long.save")

#%%
features_string = " ".join(selected_cmda_features)
print(features_string)

with open("../model/features_long.txt", "w") as f:
    f.write(features_string)


#%%
loaded_rf = joblib.load("../model/btcusdt_rf_long.save")
