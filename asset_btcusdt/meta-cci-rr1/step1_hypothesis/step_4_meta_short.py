...
short_labels['short_bin'] = 0
short_labels.loc[(short_labels['bin']==1)&(short_labels['side']==-1), 'short_bin'] = 1

short_labels['short_bin'].value_counts()

#%%
short_labels

#%%
short_labels_train = short_labels[:"2022-01-12"]
short_labels_oos   = short_labels["2022-01-14":]

#%%
# ML friendly dataset for short only model

X_train = df.loc[short_labels_train.index][trainable_features].copy()
X_train.replace([np.inf, -np.inf], np.nan, inplace=True)
X_train.fillna(0, inplace=True)
y_train = short_labels_train['short_bin']

X_test  = df.loc[short_labels_oos.index][trainable_features].copy()
X_test.replace([np.inf, -np.inf], np.nan, inplace=True)
X_test.fillna(0, inplace=True)
y_test  = short_labels_oos['short_bin']
...
return_based_sample_weights_short = get_weights_by_return(
    triple_barrier_events.loc[X_train.index], 
    df.loc[X_train.index, 'Close'],
    num_threads=1)

return_based_sample_weights_short.plot()
...
corr0, clusters, silh = cluster_kmeans_base(
    X_train.corr(), max_num_clusters=80, min_num_clusters=40, n_init=15
    )
...
importance = feature_importance_clustered_MDA(
    ...
    sample_weight=return_based_sample_weights_short, 
    scoring="f1",
    n_splits=10
    )
...
mda_clusters = [
    10,33,15,9,30,37,0,16
]
cluster_candidate_features = []
for cluster_number in mda_clusters:
    cluster_candidate_features += clusters[cluster_number]
...
cv_gen_purged = fml.cross_validation.CombinatorialPurgedKFold(n_splits=10, n_test_splits=2, samples_info_sets=triple_barrier_events.loc[X_train.index].t1, pct_embargo=0.01)
    ...
    for m_depth in parameters['max_depth']:
        for n_est in parameters['n_estimators']:
            ...
            temp_score_base = fml.cross_validation.ml_cross_val_score(clf, X_data, y_data, cv_gen, scoring=scoring,
                                                                    sample_weight_train=return_based_sample_weights_short.values)
...
parameters = {'max_depth':[20, 25, 30, 35, 40, 45],
              'n_estimators':[10, 25, 50, 100, 256, 512, 1024]}
...
probs = rf_meta.predict_proba(X_train[selected_cmda_features])[:,1]
...
probs = rf_meta.predict_proba(X_test[selected_cmda_features])[:,1]
...
calibrated_rf = CalibratedClassifierCV(rf_uncalibrated, cv=14, method="sigmoid") 
calibrated_rf.fit(
    X_train[selected_cmda_features], y_train,
    sample_weight=return_based_sample_weights_short
    )

name = "calibrated RF on test dataset"
probs = calibrated_rf.predict_proba(X_test[selected_cmda_features])[:,1]
...
frac_of_pos, mean_pred_value = calibration_curve(y_test, probs, n_bins=50)
...
#%%
df['ma_fast'] = df['Close'].rolling(30*50 ).mean()
df['ma_slow'] = df['Close'].rolling(30*200).mean()

df[['ma_slow', 'ma_fast']]

#%%
eval_features = selected_cmda_features+['Open', 'High', 'Low', 'Close', 'Volume', 'volatility_tpsl', 'ma_slow', 'ma_fast']
...
eval_df['position'] = 0
eval_df['position'] = short_labels_oos['side']
...
#%%
short_df = eval_df[eval_df['position']==-1].copy()
short_df['prob'] = 0.0
short_df['prob'] = rf_meta.predict_proba(short_df[selected_cmda_features])[:,1]
eval_df['prob'] = short_df['prob']

short_df['prob'].hist(bins=300)
...
short_df = eval_df[eval_df['position']==-1].copy()
short_df['prob'] = 0.0
short_df['prob'] = calibrated_rf.predict_proba(short_df[selected_cmda_features])[:,1]
eval_df['prob'] = short_df['prob']
...
use_kelly_betting = False

class CCIMetaShortStrategy(Strategy):
    def init(self):
        super().init()
    def next(self):
        super().next()
        ...
        probability     = self.data.prob[-1]

        if not use_kelly_betting and probability<0.55:
            return
        if use_kelly_betting and probability<0.51:
            return

        size = 0.1
        if not use_kelly_betting:
            if probability>=0.55 and probability<=0.6:
                size = 0.5
            if probability>0.6:
                size = 1.0
        else:
            size = round(kelly(probability, 1.0),2)
...
class CCIMetaShortFilteredStrategy(Strategy):
    def init(self):
        super().init()
    def next(self):
        super().next()
        ...
        volatility_tpsl = self.data.volatility_tpsl[-1]
        probability     = self.data.prob[-1]
        fast_ma         = self.data.ma_fast[-1]
        slow_ma         = self.data.ma_slow[-1]
...
joblib.dump(calibrated_rf, "../model/btcusdt_rf_short.save")

#%%
features_string = " ".join(selected_cmda_features)
print(features_string)

with open("../model/features_short.txt", "w") as f:
    f.write(features_string)


#%%
loaded_rf = joblib.load("../model/btcusdt_rf_short.save")
