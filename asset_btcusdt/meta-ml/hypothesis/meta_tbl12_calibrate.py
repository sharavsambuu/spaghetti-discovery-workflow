...
#%%
calibrated_meta_rf = CalibratedClassifierCV(meta_rf, cv='prefit', method="isotonic") 
calibrated_meta_rf.fit(
    X_test[meta_features], y_test,
    )

name = "calibrated RF on test dataset"
probs = calibrated_meta_rf.predict_proba(X_test[meta_features])[:,1]
...
#%%
"""
meta_rf_base = RandomForestClassifier(
    max_depth     = 15, 
    n_estimators  = 30,
    criterion     = 'entropy', 
...
calibrated_meta_rf = CalibratedClassifierCV(meta_rf_base, cv=8, method="isotonic") 
calibrated_meta_rf.fit(
    X_train[meta_features], y_train,
...
#%%
name = "calibrated RF on trained dataset"

probs = calibrated_meta_rf.predict_proba(X_train[meta_features])[:,1]
...
print(classification_report(
    y_test,
    calibrated_meta_rf.predict(X_test[meta_features]),
    ))

fpr_rf, tpr_rf, _ = roc_curve(y_test, calibrated_meta_rf.predict_proba(X_test[meta_features])[:, 1])
plt.figure(1)
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_rf, tpr_rf, label='RF')
plt.xlabel('False positive rate')
...
#%%
eval_df = df["2020-01-24":][eval_features].copy()
eval_df
...
joblib.dump(calibrated_meta_rf, "../model/btcusdt_rf_meta_calibrated.save")


#%%
loaded_rf = joblib.load("../model/btcusdt_rf_meta_calibrated.save")
